...
let autoReplyMessage = "📢 Привет! Это автоответчик! Сейчас или выходной, или нерабочее время. Обратитесь, пожалуйста, 📅 в рабочее время!\n\n" +
...
const sendAutoReply = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    const senderId = (_a = msg.from) === null || _a === void 0 ? void 0 : _a.id;
    ...
    if (profanityFilterEnabled && containsProfanity(messageText)) {
        ...
        const username = ((_b = msg.from) === null || _b === void 0 ? void 0 : _b.username) || "Неизвестный пользователь";
...
📈 **Предупреждения:** ${((_c = profanityWarnings.get(userId)) === null || _c === void 0 ? void 0 : _c.count) || 0}
...
⚠️ **Действия:** Если вы продолжите использовать ненормативную лексику, администрация получит уведомление, и ваш аккаунт может быть заблокирован в Telegram.`;
    ...
    if (!isWorkingDay() && autoReplyEnabled) {
        try {
            yield bot.sendMessage(chatId, autoReplyMessage);
...
const displayWarnings = (chatId) => {
    const warningsList = Array.from(profanityWarnings.values())
        .map((warning) => `👤 **Пользователь:** ${warning.username || "Неизвестный"}\n` +
        `🔑 **ID:** ${warning.id}\n` +
        `📅 **Дата:** ${warning.time}\n` +
...
const saveExcludedUsers = () => {
    try {
        fs.writeFileSync("excluded_users.json", JSON.stringify(Array.from(excludedUsers)));
...
const loadExcludedUsers = () => {
    try {
        if (fs.existsSync("excluded_users.json")) {
            const data = fs.readFileSync("excluded_users.json", "utf-8");
            excludedUsers = new Set(JSON.parse(data).filter((id) => typeof id === "number"));
            console.log("📂 Исключенные пользователи успешно загружены:", Array.from(excludedUsers));
        }
    }
    catch (err) {
...
bot.on("message", (msg) => {
    ...
    const chatId = msg.chat.id;
    const senderId = (_a = msg.from) === null || _a === void 0 ? void 0 : _a.id;
    if (!senderId || ((_b = msg.from) === null || _b === void 0 ? void 0 : _b.is_bot))
        return;
    if (waitingForExceptionInput && currentChatId === chatId) {
        const userId = parseInt((_c = msg.text) !== null && _c !== void 0 ? _c : "", 10);
        ...
        if (currentAction === "add") {
            ...
            bot.sendMessage(chatId, `✅ **Пользователь с ID ${userId} добавлен в исключения.**`);
            saveExcludedUsers();
        }
        else if (currentAction === "remove") {
            if (excludedUsers.has(userId)) {
                excludedUsers.delete(userId);
                bot.sendMessage(chatId, `✅ **Пользователь с ID ${userId} удален из исключений.**`);
                saveExcludedUsers();
            }
            else {
                bot.sendMessage(chatId, `⚠️ **Пользователь с ID ${userId} не найден в исключениях.**`);
            }
        }
    ...
    if (waitingForNewMessage && currentChatId === chatId) {
        ...
        bot.sendMessage(chatId, `✏️ **Новое сообщение для автоответчика установлено:** "${autoReplyMessage}"`);
...
bot.on("callback_query", (callbackQuery) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    switch (action) {
        case "toggle_auto_reply":
            ...
            bot.sendMessage(chatId, `✅ **Автоответчик ${autoReplyEnabled ? "включен" : "выключен"}.**`);
            break;
        case "toggle_profanity_filter":
            profanityFilterEnabled = !profanityFilterEnabled;
            bot.sendMessage(chatId, `🔒 **Фильтр мата ${profanityFilterEnabled ? "включен" : "выключен"}.**`);
            break;
        case "add_exception":
            currentAction = "add";
            waitingForExceptionInput = true;
            currentChatId = chatId;
            bot.sendMessage(chatId, "🔑 **Введите ID пользователя для добавления в исключения:**");
            break;
        case "remove_exception":
            currentAction = "remove";
            waitingForExceptionInput = true;
            currentChatId = chatId;
            bot.sendMessage(chatId, "🔑 **Введите ID пользователя для удаления из исключений:**");
            break;
        case "view_exceptions":
            bot.sendMessage(chatId, `📋 **Исключенные пользователи:** ${Array.from(excludedUsers).join(", ") || "Нет исключенных пользователей"}`);
            break;
...
bot.onText(/\/start/, (msg) => {
    const chatId = msg.chat.id;
    bot.sendMessage(chatId, "👋 **Привет! Я автоматический ответчик. Чтобы управлять пользователями, нужно узнать их ID через @userinfobot командой /start.**");
    sendMainMenu(chatId);
});
loadProfanityList();
loadExcludedUsers();
